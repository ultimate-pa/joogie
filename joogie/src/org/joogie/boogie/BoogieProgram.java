...
public class BoogieProgram {
	...
	private Variable nullVariable = new Variable("$null",
			BoogieBaseTypes.getRefType(), true);

	private Variable intArrNullVariable = new Variable("$intArrNull",
			BoogieBaseTypes.getIntArrType(), true);
	private Variable realArrNullVariable = new Variable("$realArrNull",
			BoogieBaseTypes.getRealArrType(), true);
	private Variable refArrNullVariable = new Variable("$refArrNull",
			BoogieBaseTypes.getRefArrType(), true);

	public Variable intArrSize = new Variable("$intArrSize",
			new BoogieArrayType("$intarrsizetype",
					BoogieBaseTypes.getIntType(),
					BoogieBaseTypes.getIntType()), true);

	public Variable realArrSize = new Variable("$realArrSize",
			new BoogieArrayType("$realarrsizetype",
					BoogieBaseTypes.getRealType(),
					BoogieBaseTypes.getIntType()), true);

	public Variable refArrSize = new Variable("$refArrSize",
			new BoogieArrayType("$refarrsizetype",
					BoogieBaseTypes.getRefType(),
					BoogieBaseTypes.getIntType()), true);

	public Variable stringSize = new Variable("$stringSize",
			new BoogieArrayType("$stringsizetype",
					BoogieBaseTypes.getRefType(),
					BoogieBaseTypes.getIntType()), true);

	private Variable arrSizeIdx = new Variable("$arrSizeIdx",
			BoogieBaseTypes.getIntType(), true);

	/**
	 * in the javascript tranlator, the heapvariable is overwritten
	 * by the one given by the simplified javascript
	 */
	...
	private BoogieProgram() {
		...
		// bounds
		BoogieAxiom ba = new BoogieAxiom(new BinOpExpression(Operator.Eq,
				arrSizeIdx, new UboundedIntConstant(-1L)));
	...
	public Expression getArraySizeExpression(Expression arrvar) {
		...
		if (t == BoogieBaseTypes.getIntArrType()) {
			ret = new ArrayReadExpression(this.intArrSize,
					new ArrayReadExpression(arrvar, arrSizeIdx));
		} else if (t == BoogieBaseTypes.getRealArrType()) {
			ret = new ArrayReadExpression(this.realArrSize,
					new ArrayReadExpression(arrvar, arrSizeIdx));
		} else if (t instanceof RefArrayType) {
			ret = new ArrayReadExpression(this.refArrSize,
					new ArrayReadExpression(arrvar, arrSizeIdx));
			...
			Log.error("Size of of non-array: [" + arrvar.toBoogie()
					+ "] type: [" + arrvar.getType().toBoogie() + "]");
		}
		return ret;
	}
	...
	public String toBoogie() {
		...
		if (Options.v().getHeapMode() == HeapMode.Default) {
			...
			sb.append("var " + heapVariable.toBoogie() + " : "
					+ heapVariable.getType().toBoogie() + ";\n");
		}
		sb.append("\n");

		sb.append("const unique " + nullVariable.toBoogie() + " : "
				+ nullVariable.getType().toBoogie() + " ;\n");

		if (Options.v().getHeapMode() == HeapMode.Default) {
			sb.append("const unique " + intArrNullVariable.toBoogie() + " : "
					+ intArrNullVariable.getType().toBoogie() + " ;\n");
			sb.append("const unique " + realArrNullVariable.toBoogie() + " : "
					+ realArrNullVariable.getType().toBoogie() + " ;\n");
			sb.append("const unique " + refArrNullVariable.toBoogie() + " : "
					+ refArrNullVariable.getType().toBoogie() + " ;\n");
			sb.append("\n");
		}

		sb.append("const unique " + arrSizeIdx.toBoogie() + " : "
				+ arrSizeIdx.getType().toBoogie() + ";\n");

		if (Options.v().getHeapMode() == HeapMode.Default) {
			sb.append("var " + intArrSize.toBoogie() + " : "
					+ intArrSize.getType().toBoogie() + ";\n");
			sb.append("var " + realArrSize.toBoogie() + " : "
					+ realArrSize.getType().toBoogie() + ";\n");
			sb.append("var " + refArrSize.toBoogie() + " : "
					+ refArrSize.getType().toBoogie() + ";\n");
			sb.append("\n");
			sb.append("var " + stringSize.toBoogie() + " : "
					+ stringSize.getType().toBoogie() + ";\n");
		}
		...
		for (Variable v : typeVariables) {
			sb.append("const unique " + v.toBoogie() + " : "
					+ v.getType().toBoogie() + " ;\n");
		}
		sb.append("\n");
		...
		if (Options.v().getHeapMode() == HeapMode.SimpleHeap
				&& Options.v().getEFG() != null) {
			//GlobalsCache.v().createEFGProcedure();
		}
