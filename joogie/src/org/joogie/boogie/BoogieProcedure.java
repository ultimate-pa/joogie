...
public class BoogieProcedure {
	...
	public HashSet<BoogieProcedure> calledProcedures = new HashSet<BoogieProcedure>(); // Call
																						// graph
																						// successors
	public HashSet<BoogieProcedure> callingProcedures = new HashSet<BoogieProcedure>(); // Call
																						// graph
																						// predecessors
	...
	// BinOps
	public BoogieProcedure(String name, BoogieType rettype,
			List<BoogieType> parameters, boolean ispure) {
		procedureName = name;
	 ...
	 */
	public BoogieProcedure(String name, BoogieType rettype, boolean ispure,
			boolean thisref) {
		procedureName = name;
		createReturnVar(rettype);

		if (thisref)
			thisVariable = new Variable("__this",
							BoogieBaseTypes.getRefType(), false);
		this.isPure = ispure;
	}

	public BoogieProcedure(String procName, Variable retVar,
			LocationTag tag, String nameInSource,
	...
	public BoogieProcedure(String procName, BoogieType t,
			LocationTag tag, String nameInSource,
		...
		for (BoogieType typ : parameterList2) {
			this.parameterList.add(new Variable("param"+(i++),
				BoogieBaseTypes.getRefType(), false));
		}
			...
			for (Variable v : this.tmpLocals) {
				...
				for (Expression e : exceptionReturnExpressions.values()) {
					if (e instanceof Variable
							&& ((Variable) e).toBoogie().equals(v.toBoogie())) {
