...
public class BoogieValueSwitch implements JimpleValueSwitch {
	...
	private void assertInBounds(Expression e, Expression idx) {
		guardingStatements.add(new AssertStatement(OperatorFunctionFactory
				.v().createBinOp(">=", idx,
						BoogieConstantFactory.createConst(0))));
		Expression arrsize = BoogieProgram.v()
				.getArraySizeExpression(e);
		if (arrsize != null) {
			guardingStatements.add(new AssertStatement(OperatorFunctionFactory
					.v().createBinOp("<", idx, arrsize)));
		}
	}
	...
	private void translateBinOp(BinopExpr arg0) {
		...
		expressionStack.push(OperatorFunctionFactory.v().createBinOp(
				arg0.getSymbol(), left, right));

		if (arg0.getSymbol().contains(" / ")
				&& right.getType() == BoogieBaseTypes.getIntType()) {
			this.guardingStatements.add(new AssertStatement(
					OperatorFunctionFactory.v().createBinOp("!=",
							right, BoogieConstantFactory.createConst(0))));
		}
	...
	private void translateInvokeExpr(InvokeExpr arg0, Value base) {
		...
		if (base != null) {
			...
			if (!(thisvar.getType() instanceof BoogieObjectType)) {
				...
				thisvar = OperatorFunctionFactory.v()
						.castIfNecessary(thisvar,
		...
		BoogieProcedure callee = GlobalsCache.v().lookupProcedure(
				arg0.getMethod());
	...
	public void caseStringConstant(StringConstant arg0) {
		...
		case Default:
			guardingStatements.push(BoogieHelpers.createAssignment(BoogieProgram
					.v().getStringLenExpression(stringvar),
			...
			BoogieHelpers.currentProcedure.modifiesGlobals.add(BoogieProgram
					.v().stringSize);
			break;
		case SimpleHeap:
	...
	public void caseCastExpr(CastExpr arg0) {
		Log.debug("Cast " + arg0.getOp().getType() + " to type "
	...
	public void caseInstanceOfExpr(InstanceOfExpr arg0) {
		...
		if (arg0.getCheckType() instanceof RefType) {
			te = OperatorFunctionFactory.v().createBinOp(
			...
			te = BoogieVariableFactory.v().getFreshGlobalConstant(
	...
	public void caseLengthExpr(LengthExpr arg0) {
		...
		Expression lenexp = BoogieProgram.v().getArraySizeExpression(
				exp);
		if (lenexp == null) {
			Log.error(arg0.getOp().getType().toString());
			Log.error(">> " + exp.toBoogie() + " :: "
					+ exp.getType().toBoogie());
	...
	public void caseNegExpr(NegExpr arg0) {
		arg0.getOp().apply(this);
		expressionStack.push(OperatorFunctionFactory.v().createNegOp(
				getExpression()));
	}
	...
	public void caseNewArrayExpr(NewArrayExpr arg0) {
		...
		case Default:
			expressionStack.push(BoogieVariableFactory.v()
					.getFreshHeapField(arg0.getType()));
			break;
		case SimpleHeap:
			Log.error("Warning: Arrays are not supported by the theorem prover");
			Log.error("At: " + arg0);
			expressionStack
					.push(BoogieVariableFactory.v()
							.getFreshGlobalConstant(
									BoogieTypeFactory.lookupBoogieType(arg0
											.getType())));
		}
	}
	...
	public void caseInstanceFieldRef(InstanceFieldRef arg0) {
		...
		case Default:
			...
			expressionStack.push(OperatorFunctionFactory.v()
					.createSimpleHeapAccess(
							instance,
							GlobalsCache.v().lookupField(
									arg0.getField())));
			break;
		case SimpleHeap:
			// Do a static reference
			expressionStack.push(GlobalsCache.v().lookupStaticField(
					arg0.getField()));
		}
	}
