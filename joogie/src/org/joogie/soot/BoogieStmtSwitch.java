...
public class BoogieStmtSwitch implements StmtSwitch {
	...
	/*TODO: as you can see from the, this should be renamed.
	 *  what it does is, when you have an ifthenelse, and you
	 *  create an else block and the first statement of the
	 *  else block is a jump target, we need to make sure
	 *  that jump target and the assume from the else are 
	 *  in the same block. but as our iterator cannot see
	 *  the next statement, we have to memorize it for one
	 *  round in case of else statements.
	 */
	...
	public BoogieStmtSwitch(BoogieProcedure proc) {
	...
	public void prepareCurrentBlock(Stmt stmt) {
		...
		if (stmtBlockMap.containsKey(stmt)) {
			if (this.elseBlockHack!=null && stmtBlockMap.get(stmt)!=this.elseBlockHack) {
				// in that case, we created 2 blocks for the block and need to merge them
	...
	private void translateAssigningStatement(Stmt st) {
		...
		if (st instanceof DefinitionStmt) {
			...
			if (arg0.containsInvokeExpr()
					&& arg0.getRightOp() instanceof InvokeExpr) {
				translateInvoke(arg0, arg0.getLeftOp(),
						(InvokeExpr) arg0.getRightOp());
				return;
		...
		if (rightVal instanceof NewExpr) {
			addStatement(currentBlock,
					new AssumeStatement(BoogieHelpers.isNotNull(rhs)), false);
		} else if (rightVal instanceof NewArrayExpr) {

			Expression arrsize = BoogieProgram.v()
					.getArraySizeExpression(rhs);
			if (arrsize != null) {
				NewArrayExpr narr = (NewArrayExpr) rightVal;
				narr.getSize().apply(valueSwitch);
				Expression narrsize = valueSwitch.getExpression();
				AssignStatement sizeasgn = BoogieHelpers.createAssignment(arrsize,
						narrsize);
				asgn.setLocationTag(BoogieHelpers.createLocationTag(st.getTags()));
				addStatement(currentBlock, sizeasgn, false);
				addStatement(currentBlock,
						new AssumeStatement(BoogieHelpers.isNotNull(rhs)),
						false);
			}
	...
	private void translateInvoke(Stmt st, Value assigns, InvokeExpr callExpr) {
		...
		if (callExpr.getMethod().getSignature()
			...
			Expression rhs = BoogieProgram.v()
					.getStringLenExpression(strExpr);
		...
		/*
		 * treat System.exit as a special case. Here: as a return, must be changed later.
		 */
		...
		if (e instanceof InvokeExpression) {
			...
			if (assigns != null) {
				...
				if (proc.getReturnVariable() != null) {
					returntargets.addFirst(BoogieVariableFactory.v()
							.getFreshLocalVariable(
									proc.getReturnVariable().getType()));
				}
			}
...
//			if (GlobalsCache.v().getProcedureInfo(currentProcedure).getSootMethod().getSignature()
...
//			}
			...
			if (exceptionvariables.size() > 0) {
				...
				for (Expression exc : exceptionvariables) {
					...
					addStatement(catchBlock,
							new AssumeStatement(BoogieHelpers.isNotNull(exc)),
							true);
					...
					addStatement(nothrownBlock, new AssumeStatement(
							BoogieHelpers.isNull(exc)), true);
	...
	private void translateExceptionHandling(BasicBlock b, LinkedList<Trap> traps,
			Expression exception) {
		// Check if the trap is able to handle the exception
		for (Trap trap : traps) {
			if (trap != null
					&& BoogieTypeFactory.compareTypes(((Variable) exception)
							.getType(), BoogieTypeFactory.lookupBoogieType(trap
							.getException().getType())) <= 0) {
				// trap will handle the exception
				addStatement(
						b,
						BoogieHelpers.createAssignment(GlobalsCache.v().getProcedureInfo(currentProcedure)
								.lookupLocalExceptionVar(trap), exception), false);
		...
		if (exret != null) {
			addStatement(retblock, BoogieHelpers.createAssignment(exret, exception),
					false);
		} else {				
	...
	private void translateSwitchCase(Stmt s, Expression lhs, Expression rhs) {
		...
		AssumeStatement asm = new AssumeStatement(OperatorFunctionFactory
				.v().createBinOp("==", lhs, rhs));
		...
		// we have to deepcopy!
		asm = new AssumeStatement(OperatorFunctionFactory.v()
				.createBinOp("!=", lhs, rhs));
		asm.setLocationTag(BoogieHelpers.createLocationTag(s.getTags()));
	...
	public void caseLookupSwitchStmt(LookupSwitchStmt arg0) {
		...
		while (it.hasNext()) {
			...
			Expression rhs = BoogieConstantFactory.createConst((IntConstant) it
					.next());
